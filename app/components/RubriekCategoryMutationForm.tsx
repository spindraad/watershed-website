...
type Props = DeepPartial<RubriekCategoryValidator> & {
  id?: string;
};

export default function RubriekCategoryMutationForm({
  id = '',
  ...initialValues
}: Props) {
  const { t } = useTranslation('RubriekCategoryMutationForm');
  const { SlButton } = useContext(ShoelaceContext);
  const fetcher =
    useFetcher<
      ErrorResponse<RubriekCategoryErrors, RubriekCategoryValidator>
    >();
