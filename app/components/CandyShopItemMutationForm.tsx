...
type Props = DeepPartial<Omit<CandyShopItemValidator, 'categoryId'>> & {
  id?: string;
  categories?: CandyShopCategory[];
  categoryId?: string;
};

export default function CandyShopItemMutationForm({
  id = '',
  categories = [],
  categoryId = '',
  ...initialValues
}: Props) {
  const { t } = useTranslation('CandyShopItemMutationForm');
  const { SlButton } = useContext(ShoelaceContext);
  const fetcher =
    useFetcher<ErrorResponse<CandyShopItemErrors, CandyShopItemValidator>>();
